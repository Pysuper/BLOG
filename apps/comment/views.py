...
def AddcommentView(request):
    if request.is_ajax():
        data = request.POST
        new_content = data.get('w') # 评论内容
        comment_post_ID = data.get('comment_post_ID')   # 评论对象，指的是页面留言、文章、等
        author = data.get('author', '') # 评论者
        email = data.get('email', '')   # 评论者邮箱
        url = data.get('url', '')   # 评论者网址
        ...
        CommentUser.objects.get_or_create(nickname=author, email=email, address=url)    # 存储评论者信息
        comment_parent = data.get('comment_parent') # 评论对象，父级对象，就是评论的是谁
        auser = CommentUser.objects.get(nickname=author, email=email, address=url)  # 获取用户信息
        ...
        # 存储评论信息
        if comment_post_ID == 'about':  # 关于自己页面评论
            if comment_parent == '0':   # 父级评论
            ...
            new_comment.save()
        elif comment_post_ID == 'message':  # 给我留言页面评论
            if comment_parent == '0':
